...
- **Player-specific data** (current values, current modifiers, hunger/oxygen/custom actual values) live in `[uuid].yml`; load on join, save on quit.
...
- Additional special stats: Max Hunger, Max Oxygen (defaults: 20 and 10, configurable), and custom attributes such as Max Mana (toggleable in config).
...
- Max hunger/oxygen are floats (defaults 20 and 10) with corresponding actual values stored per player.
- Visual bars show vanilla UI as a percentage of custom maxima: `20 * (real / currentMax)`, rounded up for the bar. Hunger/oxygen gain/lose events (FoodLevelChangeEvent, EntityAirChangeEvent) operate on the custom values.
