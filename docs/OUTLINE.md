...
- Normalize vanilla attributes and custom attributes, including an extended hunger cap, an oxygen bonus, and configurable custom attributes such as a Max Mana example.
...
- **Player-specific data** (current values, current modifiers, hunger/custom actual values) live in `[uuid].yml`; load on join, save on quit.
...
## Special handling: hunger and custom stats
- Max hunger is a float (default 20) with corresponding actual values stored per player.
- Visual bars show vanilla UI as a percentage of custom maxima: `20 * (real / currentMax)`, rounded up for the bar. Hunger gain/lose events (FoodLevelChangeEvent) operate on the custom values.
