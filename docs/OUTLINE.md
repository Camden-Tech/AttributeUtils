...
- Normalize vanilla attributes and custom attributes, including configurable custom attributes such as a Max Mana example.
...
- **Player-specific data** (current values, current modifiers, custom actual values) live in `[uuid].yml`; load on join, save on quit.
